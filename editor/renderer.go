...
func (editor *Editor) renderLineInInsertMode(lineIndex int, row int) {
...
func (editor *Editor) renderContentInInsertMode() {
	numberLinesToRender := editor.getNumberLinesToRender()
	for i := 0; i < numberLinesToRender; i++ {
		editor.renderLineInInsertMode(editor.renderingCursor.GetLine()+i, i)
	}
}
...
func (editor *Editor) renderContent() {
	...
	switch editor.mode {
	case INSERT_MODE:
		editor.renderContentInInsertMode()
	case SEARCH_MODE:
...
func (editor *Editor) renderInfo() {
	...
	editor.renderText(LINE_CELL_ROW+1, 0, editor.getModeAsString())
	if editor.mode == SEARCH_MODE {
		editor.renderText(PROMPT_SCREEN_LINE_BEGIN+1, PROMPT_SCREEN_COL_BEGIN, "find text: "+editor.searchParams.buffers[ON_WHICHTEXT])
	}
}
